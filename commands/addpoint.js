...
exports.run = async(bot, message, args) => {
    ...
    bot.db.getuser(target.id).then((user)=>{
        bot.db.setpoint(target.id,user.point+amount,true).then(()=>{
            message.channel.send(new Discord.MessageEmbed().setAuthor(`เพิ่ม chromos ให้ ${target.tag} จำนวน ${amount} รวม ${user.point+amount}`,'https://cdn.discordapp.com/attachments/700682902459121695/710127424943423529/text-plus-icon.png').setColor(config.colorsuccess))
        })
    })
}